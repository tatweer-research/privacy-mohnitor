...
def flan_text2text(path='alzoubi36/policy_ie_b', subtask: str = 'combined'):
    """Converts the piextract dataset to a text2text dataset."""
    ...
    for split in dataset_dict.keys():
        dataset = dataset_dict[split]
        if subtask == 'combined':
            dataset = dataset.map(lambda example: {'text': f"""Label the following tokens with the following possible labels. Mutiple labels are possible Labels: ["data-protector", "data-protected", "data-collector", "data-collected", "data-receiver", "data-retained", "data-holder", "data-provider", "data-sharer", "data-shared", "storage-place", "retention-period", "protect-against", "action", "purpose-argument", "polarity", "method", "condition-argument"] Tokens: {''.join(add_extra_ids(combine_subtasks_labels(example), mode='tokens'))}""",
                                                   'label': ''.join(add_extra_ids(example, mode='tags'))},
                                  remove_columns=subtasks)
            dataset_dict[split] = dataset
        else:
