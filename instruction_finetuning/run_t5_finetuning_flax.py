    ...
    for epoch in epochs:
        ...
        for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
            if training_args.training_mode == "pretrain":
                ...
                model_inputs = {
                    'input_ids': np.asarray([tokenized_datasets["train"][int(idx)]['input_ids'] for idx in batch_idx]),
                    'labels': np.asarray([tokenized_datasets["train"][int(idx)]['labels'] for idx in batch_idx])}
                ...
                for i, batch_idx in enumerate(tqdm(eval_batch_idx, desc="Evaluating ...", position=2)):
                    ...
                    if training_args.training_mode == "pretrain":
                        ...
                        model_inputs = {
                            ...
                            'labels': np.asarray(
                                [tokenized_datasets["validation"][int(idx)]['labels'] for idx in batch_idx])}
                        model_inputs["decoder_input_ids"] = shift_tokens_right(
                            model_inputs["labels"], data_collator.pad_token_id, data_collator.decoder_start_token_id
                        )
